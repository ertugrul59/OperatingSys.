...
char *str_append(const char *s1,const char *s2)
{
	...
	char *s_ret;
	s_ret = kmalloc(sizeof(char) * (l1 + l2 + 1), GFP_KERNEL);
	...
	if (cmd == LIST_RULES) {
		...
		for (i = 0; i < rules_len; i++) {
			if (rules[i] == '\n') {
				...
				new_rule = kmalloc(sizeof(struct f_rule), GFP_KERNEL);
...
int init_module(void)
{
	...
	errno = nf_register_hook(&firewall_ext_ops);
	if (errno) {
